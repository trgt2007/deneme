...
interface TriangleArbitrageOpportunity {
    id: string;
    tokenA: string; // Start/End token
    tokenB: string; // Intermediate token 1
    tokenC: string; // Intermediate token 2
    route: TriangleRoute;
    totalGasEstimate: bigint;
    expectedProfit: bigint;
    netProfit: bigint;
    profitMargin: number; // %
    efficiency: number; // Profit per gas unit
    confidence: number; // 0-100
    timeWindow: number; // ms
    riskScore: number; // 0-100
    complexity: number; // Route complexity score
    liquidity: bigint; // Total route liquidity
    timestamp: number;
}
...
interface TriangleRoute {
    ...
    dexDistribution: string[]; // Which DEXs used
    pathEfficiency: number; // 0-100
}

// 🦵 Triangle Leg (A→B, B→C, C→A)
...
export class TriangleArbitrage {
    ...
    private opportunities: Map<string, TriangleArbitrageOpportunity> = new Map();
    private activeExecutions: Set<string> = new Set();
    
    private metrics: TriangleMetrics = {
    ...
    private tokenCombinations: Array<{tokenA: string; tokenB: string; tokenC: string}> = [];
    ...
    constructor(config: TriangleConfig) {
        this.config = config;
        this.logger = Logger.getInstance().createChildLogger('TriangleArbitrage');
     ...
     */
    public getOpportunities(): TriangleArbitrageOpportunity[] {
     ...
     */
    public getBestOpportunity(): TriangleArbitrageOpportunity | null {
    ...
    public async executeOpportunity(
        ...
        amountIn: bigint
    ): Promise<TriangleExecutionResult | null> {
        ...
        try {
            ...
            const result: TriangleExecutionResult = {
                opportunity,
                success: true,
                actualProfit,
                gasUsed,
     ...
     */
    public async scanTriangle(
        tokenTriple: {tokenA: string; tokenB: string; tokenC: string},
        amountIn: bigint
    ): Promise<TriangleArbitrageOpportunity | null> {
        ...
        try {
            ...
            const opportunity: TriangleArbitrageOpportunity = {
                id,
     ...
     */
    public getMetrics(): TriangleMetrics {
        return { ...this.metrics };
    }
     ...
     */
    private async buildTriangleRoute(
        tokenTriple: {tokenA: string; tokenB: string; tokenC: string},
        amountIn: bigint
    ): Promise<TriangleRoute | null> {
        try {
            // Build three legs: A→B, B→C, C→A
            const legs: TriangleLeg[] = [];
            
            // Leg 1: A → B
            const leg1Quote = await this.dexAggregator.getBestQuote(
                tokenTriple.tokenA,
                tokenTriple.tokenB,
                amountIn
            );
            ...
            const leg2Quote = await this.dexAggregator.getBestQuote(
                tokenTriple.tokenB,
                tokenTriple.tokenC,
                leg1Quote.bestQuote.amountOut
            );
            ...
            const leg3Quote = await this.dexAggregator.getBestQuote(
                tokenTriple.tokenC,
                tokenTriple.tokenA,
                leg2Quote.bestQuote.amountOut
            );
            
            if (!leg3Quote) return null;
     ...
     */
    private generateTokenCombinations(): void {
        const tokens = this.config.enabledTokens;
        const combinations: Array<{tokenA: string; tokenB: string; tokenC: string}> = [];
     ...
     */
    private updateMetrics(result: TriangleExecutionResult): void {
...
// Export types
export {
    TriangleArbitrageOpportunity,
    TriangleRoute,
    TriangleLeg,
    TriangleConfig,
    TriangleMetrics,
    TriangleExecutionResult
};
